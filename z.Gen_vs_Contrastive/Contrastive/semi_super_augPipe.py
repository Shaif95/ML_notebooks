...
def color_distortion(image, s=1.0):
    ...
    def image_solarize(x):
        random_threshold = tf.random.uniform([1],minval = 0, maxval = 1)[0]
        return tf.where(x < random_threshold, x, 1 - x)
    ...
    def gauss_filter(x):
        return tfa.image.gaussian_filter2d(x, (10,10), sigma = 2.0)
    ...
    def random_overlay_color_masks(x):
        ...
        beta = tf.random.uniform(shape=[], minval=0.1, maxval=0.2)
    ...
    #image = random_apply(image_solarize, p=0.2, x = image)
    #image = random_apply(gauss_filter, p=0.2, x = image)
